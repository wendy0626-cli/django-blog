...
class Post(models.Model):
    ...
    def get_next_field_by_created_time(self):  # 获取下一页
        post = Post.objects.filter(~Q(pk=self.pk), is_show=True, post_type='post',
                                   created_time__gte=self.created_time).order_by('created_time').first()
        return post

    def get_previous_field_by_created_time(self):
        post = Post.objects.filter(~Q(pk=self.pk), is_show=True, post_type='post',
                                   created_time__lte=self.created_time).order_by('-created_time').first()
        return post
